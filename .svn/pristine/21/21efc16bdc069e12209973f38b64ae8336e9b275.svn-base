		...
		<div class="row main">
			...
			<div class="col-md-10">
				<div class="blog">
					...
					<div class="content">
						<a
							href="SelectBlogByUserIdServlet?pid=${blog.userId}&pName=${blog.userName}">${blog.userName}</a>
						<span class="text-muted"> ${blog.blogDate}</span>
						<p>${blog.blogContent}</p>
						<c:if test="${blog.userName eq user_cur.userName}">
							<a href="DeleteBlogServlet?bid=${blog.blogId}"
							 onclick="if(confirm('确认删除？')==false)return false;">删除</a>
						</c:if>
					</div>
					<form
						action="InsertCmtServlet?bid=${blog.blogId}&cmtNum=${blog.cmtNum}"
						method="post">
						<textarea class="form-control" name="cmtContent"
							required="required" placeholder="在此输入您要评论的内容..."></textarea>
						<button class="btn btn-info btn2" type="submit">评论</button>
					</form>
					<c:forEach items="${lst_cmt}" var="cmt">
						<div class="content">
							<a
								href="SelectBlogByUserIdServlet?pid=${cmt.userId}&pName=${cmt.userName}">${cmt.userName}</a>
							<span class="text-muted"> ${cmt.dayOfCmt}</span>
							<p>${cmt.cmtContent}</p>
							<c:if test="${blog.userName eq user_cur.userName}">
								<a
									href="DeleteCmtServlet?cid=${cmt.cmtId}&cmtNum=${blog.cmtNum}&bid=${blog.blogId}"
									 onclick="if(confirm('确认删除？')==false)return false;">删除</a>
							</c:if>
							<c:if test="${blog.userName ne user_cur.userName}">
								<c:if test="${cmt.userName eq user_cur.userName}">
									<a
										href="DeleteCmtServlet?cid=${cmt.cmtId}&cmtNum=${blog.cmtNum}&bid=${blog.blogId}"
										 onclick="if(confirm('确认删除？')==false)return false;">删除</a>
								</c:if>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<form
						action="InsertBlogServlet?pid=${user_cur.userId}&pName=${user_cur.userName}"
						method="post">
						<textarea class="form-control" name="blogContent"
							required="required" placeholder="在此输入您要发表的内容..."></textarea>
						<button data-dismiss="modal" class="btn btn-default btn1"
